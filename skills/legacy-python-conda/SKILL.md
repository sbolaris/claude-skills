...
description: "Write standalone Python 2.7 extraction or utility scripts against a legacy Django and pymongo codebase using an isolated conda environment."
tags: [python, python2, conda, legacy, mongodb]
---
...
Point directly at the conda env's python so the script is self-contained. Note that a shebang
is not shell-expanded — `$HOME` will **not** work there, so write out the absolute interpreter
path (`conda env list` gives it):

```python
#!/opt/miniconda3/envs/my_extract_env/bin/python
# -*- coding: utf-8 -*-
...
## Example: assay-file extraction

Real-world shape: a standalone `extract.py` in its own directory  
Conda env: `extract_env` (python=2.7, pymongo)  
Connects to: an internal MongoDB host / a collection of run records → GridFS  
Output: `files/YYYY-MM-DD/<filename>.<ext>`
